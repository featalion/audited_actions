...
6. Write your own view (and layout) if you want.
   Place view at app/views/audited_actions/audited_actions/index.html.[erb|haml]
   and layout at app/layouts/audited_actions/application.html.[erb|haml].
   ...
   Engine's controller is provided next instance variables in the view:

     @entries # => Array[AuditedActionsLogEntry, ...] - set of the log entries in the DB
     @total_entries # => Integer
     @page # => Integer, default is 1
     @per_page # => Integer, default is 10
     @total_pages # => Integer

   Accepted parameters for GET on `audited_actions` are `page` & `per_page`.

   You can launch worker from your app. Use POST to `audited_actions` with no parameters to queue worker and launch ASAP.

     <%= link_to 'Launch AuditedActions worker now!', {action: :create}, class: 'btn' %>

   To schedule new worker PUT to `audited_actions`, accepted parameter `interval` (in minutes).
